...
>
>Bob 基于 BLS 签名在他的 L1 层已经设计了一个新的优化签名方案。尤其是，他想要使用最高效形式的BLS签名聚合，这里你只需将签名加在一起就可以了，而不必对它们进行非线形聚合。为了做这件事，他基于B-KEA假设设计了一个proof-of-possession方案，这是他在Mary Maller的Sapling安全分析论文上发现的。基于Proofs-of-Possession论文的强大，他总结出他的方案也是安全的。在他部署协议之后，他发现还是被攻击了，有恶意的块进去系统，欺骗过了所有的轻节点。
...
> 
> 在密码学中，**proof of knowledge**是一种交互式证明，证明者成功地使验证者信服证明者知道某事。机器“知道某事”意味着什么，是用计算来定义的。一台机器“知道一些东西”，如果这个东西可以被计算出来，把机器作为输入。由于证明者的程序不一定会吐出知识本身（就像零知识证明一样），因此引入了具有不同程序的机器，称为知识提取器来捕获这个想法。我们最感兴趣的是多项式时间可以证明什么。在这种情况下，知识元素的集合仅限于 NP 中某种语言的一组witnesses。
...
### 2.3 聚合签名方案分析
Bob 设计了一个聚合方案来验证一组BLS签名，这组BLS签名是使用一组不同的私钥 $\{sk_i\}$ 对同一个msg的签名，假设这组签名的数量为n。首先将公钥聚合起来：
...
这个聚合方案还有一个好处，是可以在原本的聚合公钥 aggregate_key 和聚合签名 aggregate_sig 上，继续聚合任意多个新的公钥和签名，
...
```
问题代码中，首先导入一组公钥及它们的证明，共 9 （已知new_key_index = 9）个。这道题要我们在不知道这9个私钥的前提下，通过聚合一个新的公钥的签名，来构造出一个新的聚合签名，使得它能通过验证，并且这个新的公钥要能通过pok验证。

即 e(aggregate_key, hash(message)) = e($g_1$, aggregate_sig + $sig_{new}$)，

其中 aggregate_key = $\sum_{i \in [0..9)} pk_i$ + new_key = $\sum_{i \in [0..9)} [sk_i]\_1 +$ new_key = $[\sum_{i \in [0..9)} sk_i$ + sk\_new$]_1$。(new\_key = $[$sk\_new$]_1$)

虽然我们不知道 $\\{sk_i\\}\_{i \in [0, 9)}$ 的取值，但是可以通过构造 new_key 来获得任意的 aggregate_key。

令 new_key = $\[k\]\_1 - \sum_{i \in [0..9)} pk_i$，而new_key对应的私钥为 sk_new = $k - \sum_{i \in [0..9)} sk_i$。

那么aggregate_key = $\sum_{i \in [0..9)} pk_i$ + new_key = $\sum_{i \in [0..9)} \[sk\_i\]\_1 + \[k\]\_1 - \sum_{i \in [0..9)} pk_i = \[k\]\_1$，k可以是任意值。

由于我们已经知道了 ${pk_i}\_{i \in \[0, 9)}$ 的PoK证明$\{proof_i\}\_{i \in [0, 9)}$ ，因此可以利用 $\{proof_i\}\_{i \in [0, 9)}$  来构造new_key 的PoK证明。

由于 $proof_i = T \cdot {sk_i} = \[rng \cdot (i+1) \cdot {sk_i}\]\_2$，

因而 new_proof = $T_{9} \cdot$  sk_new $= \[rng \cdot 10\]\_2 \cdot (k - \sum_{i \in [0..9)} sk_i)$

$= \[rng \cdot 10 \cdot (k - \sum_{i \in \[0..9)} sk_i)\]_2$

$=\[rng \cdot 10 \cdot k\]\_2 - \sum_{i \in \[0..9)}\[rng \cdot 10 \cdot sk_i\]\_2$

$=\[rng \cdot 10 \cdot k\]\_2 -\sum_{i \in \[0..9)}\frac{10}{i+1}\[rng \cdot (i+1) \cdot sk_i\]\_2$

$=\[rng \cdot 10 \cdot k\]\_2 -\sum_{i \in \[0..9)}\frac{10}{i+1}proof_i$

接下来，为了通过BLS验证，需要生成聚合签名

aggregate_signature = $\sum_{i \in \[0..9)} sig_i$ + sig_new

= $hash(msg) \cdot {\sum_{i \in [0..9)} sk_i} + hash(msg) \cdot$ sk_new

$= hash(msg) \cdot ({\sum_{i \in [0..9)} sk_i}$ + sk_new) 
...
2.计算 new_key，new_key = $\[k\]\_1 - \sum_{i \in [0..9)} pk_i$。

3.计算 new_proof，new_proof = $\[rng \cdot 10 \cdot k\]\_2 -\sum_{i \in [0..9)}\frac{10}{i+1}proof_i$。

4.计算 aggregate_signature，aggregate_signature = $hash(msg) \cdot k$。

5.使用 new_key 和 new_proof 完成 PoK 验证。

6.计算 aggregate_key，aggregate_key = $\sum_{i \in [0..9)} pk_i +$ new_key。 
